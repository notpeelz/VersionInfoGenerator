...
namespace VersionInfoGenerator.GitInfoTask
{
    ...
    public class GetGitInfo : MarshalByRefObject, ITask
    {
        ...
        {
            {
                var (success, value, err) = await this.TryRunGitCommand("describe --long --always --dirty --exclude=* --abbrev=7");
            ...
            {
                var (success, value, err) = await this.TryRunGitCommand("describe --long --always --dirty --exclude=* --abbrev=9999");
            ...
            {
                var (success, value, err) = await this.TryRunGitCommand("rev-parse --abbrev-ref --symbolic-full-name HEAD");
            ...
            if (!string.IsNullOrEmpty(this.GitTag))
            {
                ...
                // in netstandard2.0.
                var (success, value, err) = await this.TryRunGitCommand($"rev-list \"{this.GitTag}\".. --count");
